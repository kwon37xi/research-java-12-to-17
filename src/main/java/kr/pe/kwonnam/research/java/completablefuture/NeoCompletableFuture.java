...
public class NeoCompletableFuture<T> extends CompletableFuture<T> {
    ...
    private final CompletableFuture<T> baseCompletableFuture;
    private final Executor executor;

    NeoCompletableFuture(CompletableFuture<T> baseCompletableFuture, Executor executor) {
        this.baseCompletableFuture = baseCompletableFuture;
    ...
    public CompletableFuture<T> newIncompleteFuture() {
        return new NeoCompletableFuture<>(baseCompletableFuture, executor);
    }

    public static <T> CompletableFuture<T> supplyAsync(Supplier<T> s, Executor e) {
        return neo(CompletableFuture.supplyAsync(s, e), e);
    }

    /**
     * @return 호환성을 위해 CompletableFuture 타입으로 반환할것
     */
    private static <T> CompletableFuture<T> neo(CompletableFuture<T> f, Executor e) {
        NeoCompletableFuture<T> neo = new NeoCompletableFuture<>(f, e);
        f.whenComplete((T value, Throwable throwable) -> {
            if (throwable != null) neo.completeExceptionally(throwable);
            else neo.complete(value);
        });
        return neo;
    }
}
