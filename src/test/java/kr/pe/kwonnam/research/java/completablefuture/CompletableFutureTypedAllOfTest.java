...
public class CompletableFutureTypedAllOfTest {
    ...
    void typedAllOf() throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<String> joinedCompletableFuture = allCompletableFuture.thenApply(ignoredVoid -> {
        ...
        String result = joinedCompletableFuture.get();
        long endTimestamp = System.currentTimeMillis();

        assertThat(result).as("모든 문자열이 쉼표로 조합된 상태로 반환된다.")
            .isEqualTo("하나, 둘, 셋, 넷, 다섯");
    ...
    void allOfSequentialResults() throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<String> joinedCompletableFuture = allCompletableFuture
            .thenApply(ignoredVoid -> resultQueue.stream().collect(joining(", ")));

        String result = joinedCompletableFuture.get();
        long endTimestamp = System.currentTimeMillis();

        assertThat(result).as("모든 문자열이 빠르게 실행된 순서대로 조합된 상태로 반환된다.")
            .isEqualTo("일등, 이등, 삼등, 사등, 오등");
